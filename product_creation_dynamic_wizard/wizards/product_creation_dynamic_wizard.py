...
class ProductCreationDynamicWizard(models.TransientModel):
    ...
    product_template_data = Serialized(
        default=lambda self: self._default_product_template_data()
    )
    ...
    @api.model
    def _default_product_template_data(self):
        return {
    ...
    def _compute_current_fields(self):
        for wizard in self:
            ...
            wizard.current_company_id = int(
                wizard.product_template_data.get("company_id", self.env.company.id)
            )
    ...
    def read(self, fields=None, load="_classic_read"):
        ...
        for datum in data:
            ...
            for fieldname in other_fields:
                if current_step.question_type == "field":
                    datum[fieldname] = current_wizard.product_template_data.get(
                        fieldname, False
                    )
                if current_step.question_type == "custom":
                    datum[fieldname] = (
    ...
    def write_serialized_data(self, vals):
        ...
        for rec in self:
            ...
            if vals:
                data = rec.product_template_data
                data.update(vals)
                rec.write({"product_template_data": data})
    ...
    def _is_valid_condition(self, parent_wizard_step):
        if self.step.conditional_question and parent_wizard_step:
            ...
            if parent_wizard_step.question_type == "field":
                answer = str(
                    self.product_template_data.get(parent_wizard_step.field_id.name)
                ).lower()
    ...
    def _set_product_data(self, product_data: dict):
        if self.step.question_type == "field":
            product_data[self.step.field_id.name] = self.product_template_data.get(
                self.step.field_id.name,
                product_data.get(
                    self.step.field_id.name, self.step.default_field_value
                ),
            )
            if isinstance(product_data[self.step.field_id.name], str):
                product_data[self.step.field_id.name] = product_data[
                    self.step.field_id.name
                ].replace("current_company_id", str(self.current_company_id.id))
            try:
                # useful for m2m with value such as [(0,0,{...})]
                product_data[self.step.field_id.name] = json.loads(
                    product_data[self.step.field_id.name]
                )
            except Exception:
                # TODO : consider some output while user set default values
                pass

        if self.step.question_type == "custom" and self.step.answer_id is None:
            wizard_data = self.steps
            wizard_data[self.current_step].answer_id = self.step.default_answer_id.id
        ...
        if self.step.question_type == "logical":
            ...
            for field_name in {*values.keys(), *default_values.keys()}:
                product_data[field_name] = values.get(
                    field_name,
                    self.product_template_data.get(
                        field_name,
                        default_values.get(
                            field_name,
                        ),
    ...
    def get_next_action(self):
        ...
        while not user_step:
            ...
            self._set_product_data(product_data)

            if self.step.is_automatic:
                self._save_current_step_history()
                continue
            user_step = True
        self.write_serialized_data(product_data)
        if user_step:
    ...
    def _open_wizard_action(self):
        ...
        return {
            ...
            % {
                "product_name": self.product_template_data.get(
                    "name", an_named_product_title
                )
    ...
    def _split_product_data(self):
        ...
        for fieldname, value in self.product_template_data.items():
